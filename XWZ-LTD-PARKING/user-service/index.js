...
const requireAttendant = (req, res, next) => {
  if (req.user.role !== 'attendant') {
    logger.warn(`Attendant access required, user role: ${req.user.role}`);
...
const requireAdmin = (req, res, next) => {
  if (req.user.role !== 'admin') {
    logger.warn(`Admin access required, user role: ${req.user.role}`);
...
app.post('/api/transactions/entry', authenticateToken, requireAttendant, async (req, res) => {
  try {
    ...
    if (!plateNumber || !parkingCode) {
      logger.warn('Missing required fields in entry request');
    ...
    if (parking.rows.length === 0 || parking.rows[0].availableSpaces <= 0) {
      logger.warn(`No available spaces for parking code: ${parkingCode}`);
...
app.post('/api/transactions/exit', authenticateToken, requireAttendant, async (req, res) => {
  try {
    ...
    if (parking.rows.length === 0) {
      logger.warn(`Parking not found for code: ${transaction.rows[0].parkingCode}`);
...
app.get('/api/transactions/user', authenticateToken, async (req, res) => {
  try {
    ...
    logger.info(`Fetched ${result.rows.length} transactions for user: ${req.user.id}, page: ${page}`);
    res.json({ transactions: result.rows, totalPages });
  } catch (err) {
    logger.error(`Fetch transactions error for user ${req.user.id}: ${err.message}`);
    res.status(500).json({ error: 'Server error' });
  }
});
...
app.get('/api/transactions/reports', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (!startDate || !endDate || !type) {
      logger.warn('Missing required query parameters for reports');
    ...
    if (type === 'incoming') {
      ...
      logger.warn(`Invalid report type: ${type}`);
    ...
    const totalRecords = parseInt(countResult.rows[0].count, 10);
    const totalPages = Math.ceil(totalRecords / limit);
    logger.info(`Fetched ${result.rows.length} ${type} reports for admin, page: ${page}`);
    res.json({ transactions: result.rows, totalPages });
  } catch (err) {
